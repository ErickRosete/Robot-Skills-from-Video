...
import keyboard
import time 
from utils.test_utils import init_env, viewer
...
def test_model(model, goal_path, show_goal=False, env_steps = 1000, new_plan_frec = 20 ,\
...
def test(model_name, n_mixtures, use_logistics, \
            goals, save_parent_dir="./analysis/videos/model_trials/", \
            sample_new_plan=1, n_runs=5, save_video = True, show_video = False):
    ...
    for goal,name in zip(goals,names):
        for i in range(n_runs): #save n_runs videos for each goal
            ...
            test_model(model, goal_file_path, env_steps=400, new_plan_frec=sample_new_plan, \
                        save_video=save_video, show_video = show_video,\
                        save_folder=save_parent_dir+name+"/", save_filename=video_name)
...
if __name__ == '__main__':
    ...
    save_video = True
    show_video = False
    test(model_name, num_mixtures, use_logistics, goals, \
        sample_new_plan=sample_new_plan, n_runs=n_runs, save_video= save_video, show_video=show_video)